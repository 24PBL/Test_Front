...
import { Alert, Text, View, TextInput, Button, Modal, ScrollView, TouchableOpacity, Image} from 'react-native';
...
const BirthCheck = (text) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log('이메일 보냄')
      await axios.post('http://10.0.2.2:5000/auth/join/personal/check-email', { email: inputValue });
  ...
  const nextSign= () => {
    try{
      const response = axios.post(
        'http://10.0.2.2:5000/auth/join/personal/fill-user-info',
        formData,
        {
          headers: {
  ...
  const verifyCode = async () => {
    try {
      await axios.post('http://10.0.2.2:5000/auth/join/personal/check-auth-code', {
        email: inputValue,
 ...
 const CheckPhone = (text) => {
  ...
  <TouchableOpacity onPress={pickImage}>
    <Ionicons name="camera-outline" size={35} color="#aaa" />
  </TouchableOpacity>
